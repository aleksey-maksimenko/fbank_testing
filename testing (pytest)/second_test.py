...
def driver():
    cls_url = "http://localhost:8000/?balance=30000&reserved=20001"
    options = Options()
...
def get_commission_text(driver):
    driver = driver
    try:
...
def test_commission_calculation(driver):
    ...
    for currency, xpath_card in currencies.items():
        ...
        for amount in test_amounts:
            ...
            time.sleep(0.5)
            commission_text = get_commission_text(driver)
...
def test_transfer_exceeding_balance_dollars(driver):
    """
...
def test_card_input_appears(driver):
    """
    ...
    for idx, card_xpath in enumerate(card_selectors, start=1):
        ...
        try:
            ...
            pytest.fail(f"поле 'номер карты' не найдено или не отображается при выборе счета #{idx}: {e}")

def test_amount_input_visibility(driver):
    """
    ...
    try:
        ...
        pytest.fail(f"поле суммы не найдено или не отображается при полном номере карты: {e}")
    ...
    if amount_inputs:
        if amount_inputs[0].is_displayed():
            pytest.fail("поле суммы отображается при неполном номере карты - баг")
        else:
