...
def test_valid_card_number_length(driver):
    """
...
def test_invalid_amounts(driver):
    """
...
def test_exceed_transfer_amount_euro(driver):
    """
...
def test_valid_dollar_amounts(driver):
    """
...
def test_valid_euro_amounts(driver):
    """
    ...
    for amount in valid_amounts:
        ...
        assert transfer_btn.is_enabled(), f"кнопка недоступна при сумме {amount} € — ожидался успешный перевод"
        transfer_btn.click()
        time.sleep(1)
        ...
        try:
            ...
            alert_text = alert.text
            alert.accept()
            assert "Перевод" in alert_text, f"alert не содержит подтверждения для суммы {amount} €: {alert_text}"
            print(f"[OK] перевод {amount} € выполнен — {alert_text}")
        except Exception as e:
            pytest.fail(f"[BUG] alert не появился для суммы {amount} € — {e}")
